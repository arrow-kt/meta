package meta.encoder.jvm

import arrow.core.Either
import common.utils.ClassOrPackageDataWrapper
import common.utils.ProcessorUtils
import meta.ast.Annotation
import meta.ast.Code
import meta.ast.Func
import meta.ast.PackageName
import meta.ast.Parameter
import meta.ast.Property
import meta.ast.Tree
import meta.ast.Type
import meta.ast.TypeName
...
@Suppress("LargeClass")
interface PSIElementEncoder : KotlinMetatadataEncoder, KotlinPoetEncoder, ProcessorUtils {
      ...
      val encodingResult: Either<EncodingError, Type> =
        elementUtils.getPackageOf(this@type).let { pckg ->
          ...
          when (kind) {
            ...
            else -> Either.Left(EncodingError.UnsupportedElementType("Unsupported ${this@PSIElementEncoder}, as ($kind) to Type", this@type))
          }
        }
      encodingResult.map {
        val typeElement = this@type as TypeElement
  ...
  private fun Func.maybeAsSuspendedContinuation(protoFun: ProtoBuf.Function): Func =
    if (protoFun.isSuspend) {
      copy(
        modifiers = modifiers + meta.ast.Modifier.Suspend,
  ...
  private fun TypeName.ParameterizedType.asSuspendedContinuation(): TypeName =
    when {
      isReceiverContinuation() -> TypeName.FunctionLiteral(
        modifiers = listOf(meta.ast.Modifier.Suspend),
  ...
  fun TypeElement.modifiers(): List<meta.ast.Modifier> =
  ...
  private fun Modifier.toMeta(): meta.ast.Modifier? =
    when (this) {
      Modifier.PUBLIC -> meta.ast.Modifier.Public
      Modifier.PROTECTED -> meta.ast.Modifier.Protected
      Modifier.PRIVATE -> meta.ast.Modifier.Private
      Modifier.ABSTRACT -> meta.ast.Modifier.Abstract
      Modifier.DEFAULT -> null
      Modifier.STATIC -> null
      Modifier.FINAL -> meta.ast.Modifier.Final
