...
interface TypeDecoder : MetaDecoder<Type> {
  ...
  fun Type.lyrics(): TypeSpec {
    ...
    val superclassConstructorParametersBuilder =
      superclassConstructorParameters.fold(enumConstantBuilder) { b, param ->
  ...
  fun Parameter.lyrics(): ParameterSpec {
    ...
    val builderDefaultValue =
      if (defaultValue != null) builder.defaultValue(defaultValue.lyrics()) else builder
  ...
  fun TypeName.TypeVariable.lyrics(): TypeVariableName {
    ...
    return (if (bounds.isNotEmpty()) TypeVariableName(
      this.name,
      *bounds.toTypedArray(),
      variance = variance?.lyrics()
    )
