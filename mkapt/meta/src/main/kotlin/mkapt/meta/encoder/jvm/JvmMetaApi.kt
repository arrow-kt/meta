...
import mkapt.meta.decoder.TypeDecoder
import mkapt.meta.encoder.MetaApi
import mkapt.meta.encoder.TypeClassInstance
...
interface JvmMetaApi : MetaApi, TypeElementEncoder, ProcessorUtils, TypeDecoder {
  ...
  val typeNameDownKind: (typeName: mkapt.meta.ast.TypeName) -> mkapt.meta.ast.TypeName
  ...
  override fun Func.addExtraDummyArg(): Func {
    val dummyArg: Parameter =
      Parameter("arg${parameters.size + 1}", TypeName.Unit).defaultDummyArgValue()
  ...
  override val TypeName.type: Type?
    get() = when (this) {
      is TypeName.TypeVariable      -> getTypeElement(
        name.resolveKotlinPrimitive(),
        elementUtils
      )?.asMetaType()
  ...
  override val TypeName.WildcardType.downKind: TypeName
    get() = if (upperBounds.isNotEmpty() &&
      (upperBounds.find {
        name.matches("mkapt.Kind<(\\w?), (\\w?)>".toRegex()) ||
            name.matches("mkapt.Kind<arrow.Kind<(\\w?), (\\w?)>, (\\w?)>".toRegex())
      } != null)
    ) {
      this
    } else {
  ...
  override fun TypeName.ParameterizedType.asKotlin(): TypeName.ParameterizedType =
    copy(
      name = name.asKotlin(),
      rawType = rawType.asKotlin(),
      typeArguments = typeArguments.map { it.asKotlin() })
  ...
  override fun TypeName.Classy.asKotlin(): TypeName.Classy =
    copy(
      ...
      fqName = fqName.asKotlin(),
      pckg = PackageName(pckg.value.asKotlin())
    )
    ...
    get() {
      ...
      return when {
        this is TypeName.TypeVariable &&
            (dataTypeDownKinded.simpleName.startsWith("mkapt.Kind") ||
                dataTypeDownKinded.simpleName.startsWith("arrow.typeclasses.Conested")) -> {
          simpleName
            .substringAfterLast("mkapt.Kind<")
            .substringAfterLast("mkapt.typeclasses.Conested<")
            .substringBefore(",")
            .substringBefore("<")
            ...
            val dataTypeName =
              if (dataTypeTypeArg is TypeName.TypeVariable && dataTypeTypeArg.name.contains("PartialOf<"))
                TypeName.TypeVariable(
                  dataTypeTypeArg.name.substringBefore("PartialOf<").substringAfter(
                    "<"
                  )
                )
