package mkapt.meta.encoder.jvm
...
import mkapt.common.utils.ProcessorUtils
import mkapt.meta.Either
import mkapt.meta.ast.*
import mkapt.meta.ast.Annotation
...
interface TypeElementEncoder : KotlinMetatadataEncoder, KotlinPoetEncoder, ProcessorUtils {
        ...
        elementUtils.getPackageOf(this@type).let { pckg ->
          ...
          when (kind) {
            ...
            ElementKind.CLASS     -> {
              ...
              val classBuilder =
                Type(PackageName(pckgName), asType().asTypeName().toMeta(), Type.Shape.Class)
              val declaredConstructorSignatures = meta.constructorList.map {
                it.getJvmConstructorSignature(
                  meta.nameResolver,
                  meta.classProto.typeTable
                )
              }
              val constructors =
                ElementFilter.constructorsIn(elementUtils.getAllMembers(this@type)).filter {
  ...
  fun TypeElement.properties(): List<Property> =
    meta.propertyList.map { property ->
      ...
      val jvmFieldSignature =
        property.getJvmFieldSignature(meta.nameResolver, meta.classProto.typeTable)
      val prop = Property(
  ...
  fun TypeElement.declaredFunctions(declaredElement: TypeElement): List<Func> {
    val declaredFunctionSignatures = meta.functionList.map {
      it.getJvmMethodSignature(
        meta.nameResolver,
        meta.classProto.typeTable
      )
    }
  ...
  private fun Func.maybeAsSuspendedContinuation(protoFun: ProtoBuf.Function): Func =
    if (protoFun.isSuspend) {
      copy(
        modifiers = modifiers + mkapt.meta.ast.Modifier.Suspend,
  ...
  private fun TypeName.ParameterizedType.asSuspendedContinuation(): TypeName =
    when {
      isReceiverContinuation() -> TypeName.FunctionLiteral(
        modifiers = listOf(mkapt.meta.ast.Modifier.Suspend),
  ...
  fun TypeElement.allFunctions(declaredElement: TypeElement): List<Func> =
    processorUtils().run {
      ...
      val allMembers =
        ElementFilter.methodsIn(processorUtils().elementUtils.getAllMembers(declaredElement))
      ...
      val members = filteredMembers.mapNotNull { member ->
        val templateFunction = allFunctions.find { (proto, function) ->
          function.getJvmMethodSignature(
            proto.nameResolver,
            proto.classProto.typeTable
          ) == member.jvmMethodSignature
        }
  ...
  fun TypeElement.modifiers(): List<mkapt.meta.ast.Modifier> =
  ...
  fun ExecutableElement.asConstructor(typeElement: TypeElement): Pair<Boolean, Func>? =
    kotlinMetadataUtils().run {
      typeElement.meta.constructorList.find {
        it.getJvmConstructorSignature(
          typeElement.meta.nameResolver,
          typeElement.meta.classProto.typeTable
        ) == this@asConstructor.jvmMethodSignature
      }?.let { constructor ->
  ...
  private fun Modifier.toMeta(): mkapt.meta.ast.Modifier? =
    when (this) {
      Modifier.PUBLIC       -> mkapt.meta.ast.Modifier.Public
      Modifier.PROTECTED    -> mkapt.meta.ast.Modifier.Protected
      Modifier.PRIVATE      -> mkapt.meta.ast.Modifier.Private
      Modifier.ABSTRACT     -> mkapt.meta.ast.Modifier.Abstract
      Modifier.DEFAULT      -> null
      Modifier.STATIC       -> null
      Modifier.FINAL        -> mkapt.meta.ast.Modifier.Final
