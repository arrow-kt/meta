...
import mkapt.common.Package
import mkapt.common.utils.knownError
import mkapt.common.utils.typeConstraints
import java.io.File
...
data class HigherKind(
  ...
  val alias: String = if (tparams.size == 1) "mkapt.Kind" else "mkapt.Kind${tparams.size}"
  val aliasJ: String = if (tparams.size == 1) "io.kindedj.Hk" else "io.kindedj.HkJ${tparams.size}"
  val typeArgs: List<String> = target.classOrPackageProto.typeParameters.map {
    target.classOrPackageProto.nameResolver.getString(it.name)
  }
...
class HigherKindsFileGenerator(
  ...
  fun generate() {
    higherKinds.forEachIndexed { _, hk ->
      ...
      val file = File(
        generatedDir,
        higherKindsAnnotationClass.simpleName + ".${hk.target.classElement.qualifiedName}.kt"
      )
  ...
  private fun genKindTypeAliases(hk: HigherKind): String = when {
    ...
    hk.tparams.size <= 22 -> {
      ...
      val acc =
        if (hk.tparams.size == 1) kindAlias else kindAlias + "\n" + genPartiallyAppliedKinds(hk)
      acc
    }
  ...
  private fun genPartiallyAppliedKinds(hk: HigherKind): String {
    ...
    val hkimpl =
      if (appliedTypeArgs.size == 1) "mkapt.Kind" else "mkapt.Kind${appliedTypeArgs.size}"
